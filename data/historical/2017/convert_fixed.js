...
// Load existing 2017 data
const existingDataPath = path.join(__dirname, '../election_2017.js');
const existingContent = fs.readFileSync(existingDataPath, 'utf-8');

// Extract CONSTITUENCIES_2017 object
const match = existingContent.match(/export const CONSTITUENCIES_2017 = \{([\s\S]*?)\};\s*export/);
if (!match) {
  console.error('Could not parse existing 2017 data');
  process.exit(1);
}

// Parse the data (handle missing closing brace)
const dataEndIndex = existingContent.indexOf(match[0]);
const dataString = existingContent.substring(dataEndIndex, match.index + match[0].length + 1);

let CONSTITUENCIES_2017;
try {
  // Add missing closing brace if needed
  const cleanDataString = dataString.endsWith('}') ? dataString : dataString + '}';
  eval(`CONSTITUENCIES_2017 = ${cleanDataString}`);
} catch (e) {
...
for (const [key, value] of Object.entries(CONSTITUENCIES_2017)) {
  ...
  if (province > 0) {
    const formatted = formatConstituency(key, value);
...
console.log('');
console.log(
  'Note: This is partial data (24 constituencies). Additional constituencies need to be added.'
);
