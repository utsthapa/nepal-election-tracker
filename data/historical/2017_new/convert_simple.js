...
const __dirname = '/home/lbikr/projects/nepalpoltiics/data/historical/2017_new';
...
const existingContent = fs.readFileSync(path.join(__dirname, '../election_2017.js'), 'utf-8');

// Find and extract the CONSTITUENCIES_2017 object
const startMarker = 'export const CONSTITUENCIES_2017 = {';
const startIndex = existingContent.indexOf(startMarker) + startMarker.length;

if (startIndex === -1) {
  console.error('Could not find CONSTITUENCIES_2017 in existing file');
  process.exit(1);
}

const endIndex = existingContent.indexOf('export default', startIndex);
const dataString = existingContent.substring(startIndex, endIndex);

console.log('Extracted CONSTITUENCIES_2017 object');

// Parse the object (simple JSON parse)
let CONSTITUENCIES_2017;
try {
  const objString = dataString.trim();
  eval(`CONSTITUENCIES_2017 = ${objString}`);
} catch (e) {
...
function formatConstituency(key, data) {
...
function getProvince(constituencyKey) {
  const district = constituencyKey.split('-')[0];
  return PROVINCE_MAP[district] || 0;
}

for (const [key, value] of Object.entries(CONSTITUENCIES_2017)) {
  const province = getProvince(key);
  if (province > 0) {
    byProvince[province][key] = formatConstituency(key, value);
  }
}
