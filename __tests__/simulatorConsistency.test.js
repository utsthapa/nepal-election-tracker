...
import { INITIAL_NATIONAL, OFFICIAL_FPTP_VOTE, OFFICIAL_PR_VOTE } from '../data/constituencies';
...
describe('simulator consistency', () => {
  ...
  it('baseline PR seat count should always be 110', () => {
    ...
    const prSeats = allocateSeats(nationalVoteShares, 110);
  ...
  it('combined baseline seats should always total 275 after Others fold-in', () => {
    ...
    const extraFptpSeats = Object.entries(fptpSeats).reduce(
      (sum, [party, seats]) => (party in totalSeats ? sum : sum + (seats || 0)),
      0
    );
    const extraPrSeats = Object.entries(prSeats).reduce(
      (sum, [party, seats]) => (party in totalSeats ? sum : sum + (seats || 0)),
      0
    );
