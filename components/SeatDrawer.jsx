...
export function SeatDrawer({ constituency, isOpen, onClose, onOverride, onClearOverride }) {
  ...
  const handleSliderChange = (party, value) => {
    ...
    otherParties.forEach((p, index) => {
      if (index === otherParties.length - 1) {
        newResults[p] = Math.max(
          0,
          remaining - otherParties.slice(0, -1).reduce((s, op) => s + (newResults[op] || 0), 0)
        );
      } else {
          ...
          >
            ...
            <div className="sticky top-0 bg-surface border-b border-neutral z-10">
              <div className="p-4 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-sans font-bold text-white">{constituency.name}</h3>
              ...
              {topProb && (
                <div className="px-4 py-2 flex items-center justify-between border-t border-neutral text-xs text-gray-700 bg-neutral/40">
                  <span>
                    Projected:{' '}
                    <span className={textColors[topProb[0]]}>{formatPartyLabel(topProb[0])}</span>
                  </span>
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm font-medium text-gray-700">Results Comparison</h4>
                      <div className="flex gap-3 text-xs font-semibold text-gray-500">
                        <span className="w-14 text-right">2022</span>
                        <span className="w-14 text-right text-indigo-600">2026</span>
                      </div>
                    </div>
                      ...
                      {partyOrder.map(party => {
                        ...
                        return (
                          <div key={party} className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex gap-3 font-mono text-gray-700 text-xs sm:text-sm">
                        ...
                        {partyOrder.map(party => (
                          <div key={party}>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span
                                className={`font-mono text-sm font-medium ${textColors[party]}`}
                              >
                      ...
                      <div className="mt-4 pt-4 border-t border-neutral">
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="font-mono text-white">
                            {(Object.values(localResults).reduce((a, b) => a + b, 0) * 100).toFixed(
                              2
                            )}
                            %
                          </span>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-gray-500 mt-4 pt-3 border-t border-neutral">
        ...
        <p className="mt-1">
          Showing major party candidates. Total candidates may include independents and smaller
          parties.
        </p>
      </div>
    </div>
