  ...
  return (
    <div className="relative w-full h-[600px] bg-transparent rounded-xl border border-neutral overflow-hidden">
      ...
      {hoveredConstituency && (
        ...
        >
          <div className="bg-cream-50 backdrop-blur-sm border border-stone-300 rounded-lg shadow-xl p-3 min-w-[220px]" style={{ backgroundColor: '#fefdf8' }}>
            <div className="flex items-center gap-2 mb-2">
              <div
                className="w-3 h-3 rounded-full"
                style={{ backgroundColor: PARTIES[hoveredConstituency.winner2022]?.color }}
              />
            ...
            <div className="text-xs text-gray-600 mb-2">
              {PROVINCES[hoveredConstituency.province]?.name} Province
            </div>

            <div className="border-t border-stone-200 pt-2 mt-2">
              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-600">2022 Winner:</span>
                <span
                  className="font-medium"
                  style={{ color: PARTIES[hoveredConstituency.winner2022]?.color }}
                >
                  {PARTIES[hoveredConstituency.winner2022]?.name || hoveredConstituency.winner2022}
                </span>
              </div>

              {(hoveredConstituency.candidate2022En || hoveredConstituency.candidate2022) && (
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-600">Candidate:</span>
                  <span className="text-gray-800">{hoveredConstituency.candidate2022En || hoveredConstituency.candidate2022}</span>
                </div>
              )}

              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-600">Margin:</span>
                <span className="text-gray-800">{formatPercent(hoveredConstituency.margin)}</span>
              </div>

              {hoveredConstituency.totalVotes && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Total Votes:</span>
                  <span className="text-gray-800">{hoveredConstituency.totalVotes.toLocaleString()}</span>
                </div>
              )}
            </div>

            {/* Vote share bars */}
            {hoveredConstituency.results2022 && (
              <div className="mt-3 pt-2 border-t border-stone-200">
                <p className="text-xs text-gray-500 mb-2">Vote Share</p>
                {Object.entries(hoveredConstituency.results2022)
                  .sort(([,a], [,b]) => b - a)
                  .slice(0, 4)
                  .map(([party, share]) => (
                    <div key={party} className="mb-1">
                      <div className="flex justify-between text-xs mb-0.5">
                        <span className="font-medium" style={{ color: PARTIES[party]?.color }}>
                          {PARTIES[party]?.short || party}
                        </span>
                        <span className="text-gray-600">{formatPercent(share)}</span>
                      </div>
