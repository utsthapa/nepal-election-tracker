    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-surface border border-neutral rounded-xl p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            <span
              className={`px-3 py-1 rounded-full text-sm font-bold ${
                poll.rating.startsWith('A')
                  ? 'bg-green-100 text-green-900'
                  : poll.rating.startsWith('B')
          ...
          <p className="text-lg text-gray-700 mb-6">{pollster}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-neutral rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-800">{t('polls.sampleSize')}</span>
              </div>
              <p className="text-2xl font-bold text-white">{poll.sampleSize.toLocaleString()}</p>
            </div>
            <div className="bg-neutral rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                <AlertCircle className="w-5 h-5 text-yellow-400" />
                <span className="text-sm text-gray-800">{t('polls.marginError')}</span>
              </div>
              <p className="text-2xl font-bold text-white">±{poll.marginError}%</p>
            </div>
            <div className="bg-neutral rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                <Calendar className="w-5 h-5 text-purple-400" />
                <span className="text-sm text-gray-800">{t('meta.published')}</span>
              </div>
        ...
        <div className="bg-surface border border-neutral rounded-xl p-6 mb-6">
          ...
          <p className="text-gray-700 mb-4">{methodology}</p>
          {notes && (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="font-bold text-white">{partyInfo?.name || party}</p>
                    </div>
                  </div>
