...
candidates = candidates[1:]  # Skip header row
...
for row in candidates:
    ...
    district = row[10].replace(' District', '') if len(row) > 10 else ''
    const_id = row[14] if len(row) > 14 else ''
    
    if not district or not const_id:
        continue
    ...
    # Parse party
    party_name = row[8] if len(row) > 8 else ''
    party_id = 'Others'
    ...
    # Parse votes
    try:
        votes = int(row[16]) if len(row) > 16 and row[16] else 0
    except:
        votes = 0
...
# Keep the header and comments
header = lines[:start_idx]

# Extract the array content (from "export const constituencies = [" to "];")
array_content = ''.join(lines[start_idx-1:end_idx])  # -1 to include the line with "export const..."

# Remove the "export const constituencies = " part and extract just the array
...
for const in constituencies:
    ...
    if match:
        ...
        const_key = (district, str(const_id))
...
# Add the new array declaration
array_line = 'export const constituencies = ' + new_array_str + ';\n'
# No content after the array (file ends with the array)
