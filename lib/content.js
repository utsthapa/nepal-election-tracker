...
export function getArticlesFromDir(dir) {
  const fullPath = path.join(CONTENT_DIR, sanitizePath(dir));
  ...
  const articles = files
    .filter(file => (file.endsWith('.mdx') || file.endsWith('.md')) && !file.endsWith('.bak'))
    .map(file => {
...
export function getArticleBySlug(dir, slug) {
  const fullPath = path.join(CONTENT_DIR, sanitizePath(dir), `${sanitizePath(slug)}.mdx`);
